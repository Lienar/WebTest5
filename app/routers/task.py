...
async def create_task(db: Annotated[Session, Depends(get_db)], create_task: CreateTask, user_cr: str):
    user = db.scalars((select(User).where(User.username == user_cr))).one_or_none()
    if user is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
